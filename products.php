...
<style>
      ...
      <div class="pagetitle">
        ...
        <nav>
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item ">Listes Produit</li>
          </ol>
        </nav>
      </div>
      <!-- End Page Title -->
  <?php
   ...
   if(isset($_POST['clique'])){
      $var = $_POST['chercher'];
      $req = "SELECT * FROM `produit` WHERE Nomprod LIKE '%$var%'";
      $select = mysqli_query($con,$req);
   }else{
      $select = mysqli_query($con, "SELECT * FROM produit");
   }
   ?>
...
<div class="card">
         ...
         <tr>
            <td><img src="images/<?php echo $row['imageP']; ?>" class="product-image" alt=""></td>
            <td><?php echo $row['Nomprod']; ?></td>
            <td><?php echo $row['qte']; ?></td>
            <td><?php echo $row['prix']; ?> DH</td>
            <td>
               <button class="btn btn-warning" onclick="openUpdatePopup(<?php echo $row['idP']; ?>)">
        <i class="fas fa-edit"></i>
                </button>
    
    <a href="javascript:void(0);" onclick="confirmDelete(<?php echo $row['idP']; ?>)" class="btn btn-danger">
...
function openUpdatePopup(productId) {
    ...
    var updateUrl = 'updateP.php?edit=' + productId;

    // Ouvrir une nouvelle fenêtre de modification
    window.open(updateUrl, 'Update Product', 'width=600,height=400');
}
</script>
...
<script>
    function confirmDelete(productId) {
        if (confirm("Voulez-vous vraiment supprimer ce produit?")) {
            window.location.href = 'product.php?delete=' + productId;
        }
    }
</script>
